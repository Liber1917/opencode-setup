...
6. 远程仓库/知识调研默认栈(免费)= gh CLI/API(结构/文件/搜索,匿名 60 次/时、带号 5000)+ raw.githubusercontent.com + 通用 web 搜索;本会话实测 zread 未收录 4/5 仓库时降级该栈完成同等调研。付费/半付费增强(zread 等)不进依赖链,仅用户自带 key 可选——与 DCP/MinerU 的"付费不推荐"立场一致
